...
df.to_csv('Results/Execution_Results.csv',index=False)
...
plt.plot(df['Threads'].values[1:],df['Parallelization_Fraction'].values[1:],linestyle='solid',color='black', lw=1.5)
plt.scatter(df['Threads'].values[1:],df['Parallelization_Fraction'].values[1:],color='r',zorder=1)
plt.rcParams["figure.figsize"] = (20,20)
plt.xticks(np.arange(np.min(df['Threads']), np.max(df['Threads'])+1, 10))
plt.yticks(np.arange(1, np.max(df['Parallelization_Fraction'].values[1:]), 10))
plt.savefig('Results/threads_vs_parallelization_fraction.png')